...
Solution --- Here the idea is we will get the total and take avg then we will try to find all the subarray are create their sum and we know that the 
remaining sum will total - subarraysum , then we will compare the average of both.

Time o(n2)
space 0(n)
 
Since you need to actually output the indices, you can't do better than O(n^2) since in the worst case, there are order n^2 subarrays in the answer: 
consider an array of N-1 1000 followed by 1 1.Any subarray of the contiguous portion of 1000s is a valid solution, and there are order n^2 of them.
